...
%%--------------------------------------------------------------------
init([]) ->
    lager:info("starting remote dispatcher at ~p", [self()]),
    {ok, #state{}}.
...
%% @end
%%--------------------------------------------------------------------
handle_call({fetch, Path, From, Until, Now}, _From, State) ->
    Reply = fetch(Path, From, Until, Now),
    {reply, Reply, State}.
